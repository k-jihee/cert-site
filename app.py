...
def generate_qr_image_bytes(url: str, box_size: int = 10, border: int = 4) -> bytes:
...
def create_qr_zip_from_excel(
    ...
    if missing_columns:
        raise ValueError(f"엑셀에 필요한 컬럼이 없습니다: {', '.join(missing_columns)}")

    if "product_name" not in excel_df.columns:
        excel_df["product_name"] = excel_df["product_code"]

    excel_df["product_code"] = excel_df["product_code"].astype(str).str.strip()
    excel_df["product_name"] = excel_df["product_name"].astype(str).str.strip()
    excel_df = excel_df[excel_df["product_code"] != ""].drop_duplicates(subset=["product_code"], keep="first")
    ...
    for _, row in excel_df.iterrows():
        ...
        safe_name = "".join(ch if ch.isalnum() or ch in ("-", "_") else "_" for ch in product_code)
        file_name = f"{safe_name}_QR.png"
        documents.append((file_name, qr_bytes))
        result_rows.append({
    ...
    return build_zip_from_documents(documents), pd.DataFrame(result_rows)
...
if product_code:
    ...
    with st.expander("엑셀 기반 QR 일괄 생성", expanded=False):
        st.write("product_code 컬럼이 포함된 엑셀 파일을 업로드하면 제품별 QR PNG를 ZIP으로 일괄 생성합니다. product_name 컬럼은 선택입니다.")
        sample_df = pd.DataFrame({
            "product_code": ["corn-starch", "sun-pregel"],
            "product_name": ["Corn Starch", "Sun Pregel"],
        })
        ...
        st.download_button(
            ...
            file_name="qr_upload_template.xlsx",
        ...
        if uploaded_excel is not None:
            try:
                qr_zip_buffer, qr_result_df = create_qr_zip_from_excel(uploaded_excel)
                st.success(f"총 {len(qr_result_df)}개 제품의 QR을 생성했습니다.")
                st.dataframe(qr_result_df, use_container_width=True)
                st.download_button(
                    label="QR ZIP 다운로드",
                    data=qr_zip_buffer.getvalue(),
                    file_name="product_qr_codes.zip",
                    mime="application/zip",
                )
            except Exception as e:
                st.error(f"QR 생성 중 오류가 발생했습니다: {e}")
